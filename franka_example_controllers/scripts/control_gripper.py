...
def sub_gripper(gp, pos, dt_sec):
    ...
    control = MoveActionGoal()
    control.goal.width = pos
    control.goal.speed = 0.1

    if (gp == 'l' or gp == 'L'):
        pubA = rospy.Publisher('/panda_left/franka_gripper/move/goal', MoveActionGoal, queue_size=1, latch=True)
        pubA.publish(control)
    elif (gp == 'r' or gp == 'R'):
        pubB = rospy.Publisher('/panda_right/franka_gripper/move/goal', MoveActionGoal, queue_size=1, latch=True)
